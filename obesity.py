...
# 📥  Carregar o dataset
df = pd.read_csv("datasets/obesity/ObesityPrediction.csv", nrows=800)

# Selecionar apenas colunas relevantes e tratar valores faltantes
df = df[['Obesity', 'Age', 'Height', 'Weight', 'Gender', 'family_history', 'FAVC', 'FCVC', 'NCP', 'CAEC', 'SMOKE', 'CH2O', 'SCC', 'FAF', 'TUE', 'CALC', 'MTRANS']].dropna()
...
# Normalizar apenas as colunas numéricas (Age, Height, Weight, FCVC, NCP, CH2O, FAF, TUE)
scaler = MinMaxScaler()
...
mlp_relu = MLPClassifier(hidden_layer_sizes=(100,), activation="relu", max_iter=5000, random_state=42)
mlp_tanh = MLPClassifier(hidden_layer_sizes=(100,), activation="tanh", max_iter=5000, random_state=42)
mlp_relu_large = MLPClassifier(hidden_layer_sizes=(200, 100), activation="relu", max_iter=5000, random_state=42)
mlp_tanh_large = MLPClassifier(hidden_layer_sizes=(200, 100), activation="tanh", max_iter=5000, random_state=42)
